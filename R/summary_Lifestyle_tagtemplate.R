#' Summary of GNPS Lifestyle Tags - Tag Template
#'
#' This function works to generate a summary of the GNPS Lifestyle tags associated with annotated chemicals resulting from spectral library matching in GNPS. Users can choose to add new tags using the GNPS tag template. The GNPS tag template can be downloaded and used to produce the summary plots. The GNPS Lifestyle tags are associated with planar InChIKeys.
...
summary_Lifestyle_tagtemplate <- function(){
  ...
  tags <- cbind(df_master$InChI_Key_Planar,separate(df_master[,"Lifestyle_Tag"], Lifestyle_Tag, paste0("X",1:max(sapply(strsplit(df_master$Lifestyle_Tag,"\\|"),length))), sep="\\|"))
    studyinfo_n_row_annotations <- as.numeric(length(df_tagtemplate$`INPUT-#Scan#`)) #numbers out
  colnames(tags)[1] <- "InChI_Key_Planar"
  add_tags <- df_tagtemplate %>% select(`INPUT-InChIKey-Planar`,`Add_InCHIKey-Planar`,`Contribute-LifestyleTag1`,`Contribute-LifestyleTag2`,`Contribute-LifestyleTag3`)
  ...
  if (sum(df_master_CCMSLIB$Lifestyle_Tag != "") > 0) {
    tags <- cbind(df_master_CCMSLIB$CCMSLIB,separate(df_master_CCMSLIB[,"Lifestyle_Tag"], Lifestyle_Tag, paste0("X",1:max(sapply(strsplit(df_master_CCMSLIB$Lifestyle_Tag,"\\|"),length))), sep="\\|"))
  ...
  add_tags <- df_CCMSLIB_tags %>% select(`INPUT-SpectrumID`,`Contribute-LifestyleTag1`,`Contribute-LifestyleTag2`,`Contribute-LifestyleTag3`)
  ...
  print(plot_summary)
  ggsave(file="GNPS_TagTemplate_Summary_Lifestyle.pdf", width = 3.25, height = 3.25, units = "in", dpi=300, useDingbats=FALSE)
  ggsave(file="GNPS_TagTemplate_Summary_Lifestyle.png", width = 3.25, height = 3.25, units = "in")
  ...
  print(plot_summary)
  ggsave(file="GNPS_TagTemplate_SummaryPercentage_Lifestyle.pdf", width = 3.25, height = 3.25, units = "in", dpi=300, useDingbats=FALSE)
  ggsave(file="GNPS_TagTemplate_SummaryPercentage_Lifestyle.png", width = 3.25, height = 3.25, units = "in")
  ...
  zipr("UBERON_zip.zip", c("GNPS_hittable_LifestyleTag.csv",
                           "GNPS_TagTemplate_Summary_Lifestyle.pdf", 
                           "GNPS_TagTemplate_Summary_Lifestyle.png",
                           "GNPS_TagTemplate_SummaryPercentage_Lifestyle.pdf", 
                           "GNPS_TagTemplate_SummaryPercentage_Lifestyle.png",
