#' Summary of GNPS Tags - Tag Template
#'
#' This function works to generate a summary of the GNPS Tags associated with annotated chemicals resulting from spectral library matching in GNPS. Users can choose to add new tags using the GNPS tag template. The GNPS tag template can be downloaded and used to produce the summary plots. Tags are associated with planar InChIKeys.
...
summary_TAGS_tagtemplate <- function(){
  ...
  tags <- cbind(df_master$InChI_Key_Planar,separate(df_master[,"TAGS"], TAGS, paste0("X",1:max(sapply(strsplit(df_master$TAGS,"\\|"),length))), sep="\\|"))
  colnames(tags)[1] <- "InChI_Key_Planar"
  add_tags <- df_tagtemplate %>% select(`INPUT-InChIKey-Planar`,`Add_InCHIKey-Planar`,Add_Tag1,Add_Tag2,Add_Tag3)
  ...
  tags <- cbind(df_master_CCMSLIB$CCMSLIB,separate(df_master_CCMSLIB[,"TAGS"], TAGS, paste0("X",1:max(sapply(strsplit(df_master_CCMSLIB$TAGS,"\\|"),length))), sep="\\|"))
  ...
  add_tags <- df_CCMSLIB_tags %>% select(`INPUT-SpectrumID`,Add_Tag1,Add_Tag2,Add_Tag3)
  ...
  write.csv(hit_table, "GNPS_hittable_TAGS.csv", row.names=FALSE)
  ...
  plot_summary <- ggplot(data=hit_table, aes(x= reorder(as.factor(terms),n), y=as.numeric(n)))+
    ...
    labs(x="GNPS Tags", y="Number of Tags")
  print(plot_summary)
  ggsave(file="GNPS_TagTemplate_Summary_TAG.pdf", width = 3.25, height = 3.25, units = "in", dpi=300, useDingbats=FALSE)
  ggsave(file="GNPS_TagTemplate_Summary_TAG.png", width = 3.25, height = 3.25, units = "in")
  ...
  plot_summary <- ggplot(data=hit_table, aes(x= reorder(as.factor(terms),percentchemicalstagged), y=as.numeric(percentchemicalstagged)))+
    ...
    theme(panel.grid.major=element_line(colour ="grey75",size=0.25, linetype="dotted"),
          ...
          legend.justification = c(1,0),
          legend.position = c(1,0))+
    labs(x="GNPS Tags", y="% of Unique Planar InChIKeys or CCMSLIB IDs")
  print(plot_summary)
  ggsave(file="GNPS_TagTemplate_SummaryPercentage_TAG.pdf", width = 3.25, height = 3.25, units = "in", dpi=300, useDingbats=FALSE)
  ggsave(file="GNPS_TagTemplate_SummaryPercentage_TAG.png", width = 3.25, height = 3.25, units = "in")
