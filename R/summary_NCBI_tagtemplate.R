#' Summary of NCBI Taxonomy Tags in GNPS - Tag Template
#'
#' This function works to generate a summary of the NCBI taxonomy tags associated with annotated chemicals resulting from spectral library matching in GNPS. Users can choose to add new tags using the GNPS tag template. The GNPS tag template can be downloaded and used to produce the summary plots. NCBI taxonomy tags are associated with planar InChIKeys.
...
summary_NCBI_tagtemplate <- function(){
  ...
  tags <- cbind(df_master$InChI_Key_Planar,separate(df_master[,"NCBITaxonomy"], NCBITaxonomy, paste0("X",1:max(sapply(strsplit(df_master$NCBITaxonomy,":"),length))), sep=":"))
    studyinfo_n_row_annotations <- as.numeric(length(df_tagtemplate$`INPUT-#Scan#`)) #numbers out
  colnames(tags)[1] <- "InChI_Key_Planar"
  add_tags <- df_tagtemplate %>% select(`INPUT-InChIKey-Planar`,`Add_InCHIKey-Planar`,`Contribute-NCBITaxonomy1`,`Contribute-NCBITaxonomy2`,`Contribute-NCBITaxonomy3`)
  ...
  if (sum(df_master_CCMSLIB$NCBITaxonomy != "") > 0) {
    tags <- cbind(df_master_CCMSLIB$CCMSLIB,separate(df_master_CCMSLIB[,"NCBITaxonomy"], NCBITaxonomy, paste0("X",1:max(sapply(strsplit(df_master_CCMSLIB$NCBITaxonomy,":"),length))), sep=":"))
  ...
  add_tags <- df_CCMSLIB_tags %>% select(`INPUT-SpectrumID`,`Contribute-NCBITaxonomy1`,`Contribute-NCBITaxonomy2`,`Contribute-NCBITaxonomy3`)
  ...
  if (nrow(df_CCMSLIB_tags) > 0) {
    ...
    tags_CCMS <- cbind(df_CCMSLIB_tags[,"CCMSLIB"],separate(df_CCMSLIB_tags[,"NCBITaxonomy"], NCBITaxonomy, paste0("X",1:max(sapply(strsplit(df_CCMSLIB_tags$NCBITaxonomy,":"),length))), sep=":"))
  ...
  write.csv(hit_table, "GNPS_hittable_NCBITaxonomyTag.csv", row.names=FALSE)
  ...
  plot_summary <- ggplot(data=hit_table, aes(x= reorder(as.factor(terms),n), y=as.numeric(n)))+
    ...
    labs(x="NCBI Taxonomy Tags", y="Number of Tags")
  print(plot_summary)
  ggsave(file="GNPS_TagTemplate_Summary_NCBI.pdf", width = 3.25, height = 3.25, units = "in", dpi=300, useDingbats=FALSE)
  ggsave(file="GNPS_TagTemplate_Summary_NCBI.png", width = 3.25, height = 3.25, units = "in")
  ...
  plot_summary <- ggplot(data=hit_table, aes(x= reorder(as.factor(terms),percentchemicalstagged), y=as.numeric(percentchemicalstagged)))+
    ...
    theme(panel.grid.major=element_line(colour ="grey75",size=0.25, linetype="dotted"),
          ...
          legend.justification = c(1,0),
          legend.position = c(1,0))+
    labs(x="NCBI Taxonomy Tags", y="% of Unique Planar InChIKeys or CCMSLIB IDs")
  print(plot_summary)
  ggsave(file="GNPS_TagTemplate_SummaryPercentage_NCBI.pdf", width = 3.25, height = 3.25, units = "in", dpi=300, useDingbats=FALSE)
  ggsave(file="GNPS_TagTemplate_SummaryPercentage_NCBI.png", width = 3.25, height = 3.25, units = "in")
